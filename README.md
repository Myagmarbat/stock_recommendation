...
- Pulls top 50 symbols from Yahoo Finance day gainers.
...
- Runs every 10 minutes in background via `launchd` during market hours only (weekdays).
- Self-adjusts model weights/thresholds after each run based on prior pick correctness.
- Applies 10-minute feedback learning: if last run calls were wrong, logic is updated for the next run.
...
- Tracks virtual portfolio performance from an initial `$10,000` and updates equity every run.
- Generates one end-of-day strategy summary (weekdays, after 4:00 PM ET): daily gain/loss, recommendation effectiveness, and improvement notes.
...
- Resets open positions and run counters to align simulation with your real balance baseline
...
- Initial capital: `$10,000` (virtual portfolio).
...
## Run every 10 minutes in background (macOS)
