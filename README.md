...
Automated stock and ETF scanner with deterministic trade scoring, optional AI review, local RAG over generated outputs, paper portfolio tracking, and background scheduling.

## Architecture

The project has four main layers:

1. Deterministic scanner and trading engine
2. Optional OpenAI/LangChain advisor
3. Local RAG over docs and generated outputs
4. Scheduler and storage layer
...
Yahoo Finance market data/news
  ...
  -> top candidates, trade levels, sizing, alerts, paper portfolio
  -> optional OpenAI advisor review
  -> markdown/csv/json outputs under data/
  -> optional local FAISS/Ollama RAG for Q&A over outputs
```

The deterministic engine is the source of trade decisions. It collects prices, fundamentals, news headlines, market regime, category trends, technical signals, earnings catalysts, and portfolio state. Python computes scores, actions, entry/target/stop levels, risk/reward, sizing, alerts, and paper-trading updates.

The OpenAI/LangChain layer is advisory. It reviews the structured top-pick payload and writes AI fields such as confidence, risk level, reason, and improvement notes. In the default `advisory` mode, it does not change trade actions. Cost controls live in `config/agent_config.json` under `ai`.

The local RAG layer is separate from the paid advisor. It uses `sentence-transformers` locally for embeddings, FAISS for the vector index, and Ollama for local answers. It is intended for questions over local docs, reports, AI decisions, run history, and portfolio outputs. It does not use OpenAI embeddings or a paid hosted vector database.

Scheduler flow:

```text
launchd / cron / Task Scheduler
  -> scripts/run_agent.sh or scripts/run_agent.ps1
  -> load .env
  -> run stock_option_agent/agent.py
  -> write partitioned outputs to data/daily/YYYYMMDD/
  -> refresh data/today pointer
```

Storage flow:

```text
config/                 static config and profiles
data/daily/YYYYMMDD/    primary run outputs and logs
data/today              symlink/junction to current day
data/portfolio/         persistent paper-trading state
data/model/             adaptive scoring state
data/rag/faiss/         local FAISS vector index
```

`data/` is generated runtime state and is gitignored.
...
export STOCK_RECOMMENDATION_AI_MAX_OUTPUT_TOKENS=1200
```
...
The project can build a local RAG index over PDFs, documentation, config, and generated scanner outputs.

This RAG path is local-only:
...
- Answers are generated by local Ollama

Flow:

```text
PDFs / docs / README / run outputs
  -> split into chunks
  -> create embeddings locally with sentence-transformers
  -> store embeddings in FAISS
  -> user asks a question
  -> search similar chunks from FAISS
  -> send chunks + question to local Ollama
...
Build or replace the FAISS index. On the first run, allow downloading the local embedding model from Hugging Face:

```bash
python -m stock_recommendation.rag ingest --allow-model-download
```

After the model is cached locally, omit `--allow-model-download` for offline/local runs.
...
```text
data/rag/faiss/index.faiss
data/rag/faiss/metadata.json
```

Recommended local environment flag:

```bash
export TOKENIZERS_PARALLELISM=false
```

Retrieve matching context without an LLM answer:

```bash
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag query "Why did the last run choose GOOGL?"
```

Retrieve context and ask the LLM to answer from it:

```bash
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag ask "Summarize the latest AI decision and cite the source files"
```

Use custom sources or index location:

```bash
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag ingest --source data/latest --source data/ai --index-dir data/rag/faiss
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag query "What changed in today's recommendations?" --index-dir data/rag/faiss --top-k 8
```

Use a different local embedding or Ollama model:

```bash
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag ingest --embedding-model sentence-transformers/all-mpnet-base-v2 --allow-model-download
TOKENIZERS_PARALLELISM=false python -m stock_recommendation.rag ask "What changed today?" --ollama-model llama3.2
```

Generated FAISS files live under `data/`, which is gitignored.
